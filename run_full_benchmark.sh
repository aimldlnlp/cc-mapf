...
for run_dir in "$RESULTS_DIR"/*/*/; do
    if [ -f "$run_dir/results.json" ]; then
        ...
        python render_advanced_visualizations.py "$run_dir" figures 2>&1 >> "$LOGFILE" || true
    fi
done
...
if [ -n "$first_run" ]; then
    ...
    python render_showcase.py "$first_run" "$RESULTS_DIR/showcase" 2>&1 >> "$LOGFILE" || true
...
tmux new-session -d -s mapf-benchmark "bash /tmp/benchmark_inner.sh $RESULTS_DIR"
