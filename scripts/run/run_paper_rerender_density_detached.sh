...
LOG_FILE="$LOG_DIR/paper_rerender_density_${STAMP}.log"
META_FILE="$LOG_DIR/paper_rerender_density_latest.env"
...
cat > /tmp/run_paper_rerender_density_inner.sh <<'INNER'
#!/bin/bash
set -euo pipefail
...
summary["comparison_run_note"] = "Comparison subset was regenerated for PNG density rerender and then cleaned up."
...
status["comparison_run_note"] = "Comparison subset was regenerated for PNG density rerender and then cleaned up."

manifest_path = bundle_output_dir / "paper_bundle_manifest.json"
manifest_payload = json.loads(manifest_path.read_text(encoding="utf-8"))
manifest_payload["metadata"]["comparison_run"] = None
manifest_payload["metadata"]["comparison_run_note"] = "Comparison subset was regenerated for PNG density rerender and then cleaned up."
...
PY
  ...
  echo
  echo "========================================"
  echo "Density rerender complete"
...
chmod +x /tmp/run_paper_rerender_density_inner.sh
...
tmux new-session -d -s "$SESSION_NAME" "bash /tmp/run_paper_rerender_density_inner.sh '$ROOT_DIR' '$ROLLOUT_DIR' '$OFFICIAL_RUN' '$COMPARISON_CONFIG' '$RENDER_CONFIG' '$LOG_FILE' '$META_FILE' '$SESSION_NAME'"

echo "Started detached density rerender session."
